...
params['ModelParams']['prototxtTrain'] = os.path.join(basePath, 'models/train_noPooling_ResNet_cinque.prototxt')
params['ModelParams']['prototxtTest'] = os.path.join(basePath, 'models/test_noPooling_ResNet_cinque.prototxt')
params['ModelParams']['snapshot'] = 0
params['ModelParams']['dirTrain'] = os.path.join(basePath, 'data/train')
params['ModelParams']['dirTest'] = os.path.join(basePath, 'data/test')
params['ModelParams']['dirResult'] = os.path.join(basePath, 'results') # where we need to save the results (relative to the base path)
params['ModelParams']['dirSnapshots'] = os.path.join(basePath, 'snapshots/') # where to save the models while training
